...
describe("when blogs are initialized", () => {
  ...
  describe("adding a blog", () => {
    ...
    test("succeeds with valid data", async () => {
      ...
      expect(titles).toContain("sample blog entry");
    });
    ...
    test("likes property defaults to zero if not defined", async () => {
      ...
      const likesProperty = Object.values(
        response.body[helper.initialBlogs.length]
      )[3];
  ...
  describe("deleting a blog", () => {
    beforeEach(async () => {
      // add a test user so we can create a blog with a user id
      await User.deleteMany({});

      const testUser = await helper.testUser();
      await testUser.save();

      // add a test blog to delete
      await Blog.deleteMany({});
      const testBlog = await helper.blogWithToken();
    ...
    test("a blog can be deleted", async () => {
      ...
      const blogToRemove = testBlogs[0];

      const token = await helper.testUserToken();

      await api
        .delete(`/api/blogs/${blogToRemove._id.toString()}`)
        .set("Authorization", `Bearer ${token}`)
        .expect(204);

      const endBlogs = await helper.blogsInDb();

      expect(endBlogs).toHaveLength(testBlogs.length - 1);
...
describe("initialize database with one user", () => {
  ...
  test("creation fails with proper status code and message if username has invalid length", async () => {
    ...
    expect(result.body.error).toContain(
      "User validation failed: username: must be at least 3 characters or more"
    );
  ...
  test("creation fails with proper status code and message if password has invalid length", async () => {
    ...
    expect(result.body.error).toContain(
      "password must be at least 3 characters or more"
    );
